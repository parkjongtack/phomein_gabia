...
<html lang="ko">
<head>
<title>자료실 | 인트라넷 | 포메인 통합관리자</title>
	...
	<section id="dBody">
		...
		<section id="contents">
			...
			<div class="default-cell">
				<h4 class="sub-title">자료실 등록<span class="fc01">* 는 필수입력/선택 항목입니다</span></h4>
				...
				<div class="table-list-data">
					<fieldset>
						<legend>자료실 등록 기본정보 입력 폼</legend>
						<table class="view">
							<caption>자료실 등록 기본정보 입력 폼 테이블</caption>
							<colgroup>
							...
							<tbody>
								...
								<tr>
									...
									<td><input type="text" name="title" style="width:100%"  data-null="not" title="제목"  placeholder="제목을 입력하세요"></td>
								</tr>
								<tr>
									<th scope="row">내용 <span class="need"></span></th>
									<td><div class="edit-area"><textarea id="conts" name="conts" rows="3" data-null="not" title="내용" style="width:100%;" ></textarea></div></td>
								</tr>
								<tr>
...
$("._addBtn").click(function() {
    ...
    */
    ...
    if ($("input[name=file-name01]").val() !== undefined) {
    ...
    if ($("input[name=file-name02]").val() !== undefined) {
	   	if($("input[name=file-name02]").val() === ''){
	   		alert('첨부파일을 등록해주세요.');
	   		return false;
	   	} else {
    ...
    if ($("input[name=file-name03]").val() !== undefined) {
	   	if($("input[name=file-name03]").val() === ''){
	   		alert('첨부파일을 등록해주세요.');
	   		return false;
	   	} else {
    ...
    if ($("input[name=file-name04]").val() !== undefined) {
	   	if($("input[name=file-name04]").val() === ''){
	   		alert('첨부파일을 등록해주세요.');
	   		return false;
	   	} else {
    ...
    if ($("input[name=file-name05]").val() !== undefined) {
	   	if($("input[name=file-name05]").val() === ''){
	   		alert('첨부파일을 등록해주세요.');
	   		return false;
	   	} else {
