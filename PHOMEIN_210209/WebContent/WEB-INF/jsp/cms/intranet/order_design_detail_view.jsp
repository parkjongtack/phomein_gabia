...
<html lang="ko">
<head>
<title>디자인발주신청내역 | 인트라넷 | 포메인 통합관리자</title>
...
<body>
	...
	<div id="wrap">
		...
		<section id="dBody">
			...
			<section id="contents">
				<!-- location -->
				<p class="location">
					<span>인트라넷</span> <span>디자인발주신청내역</span> <span>상세/답글</span>
				</p>
				<!-- //location -->
				<h3 class="con-title">디자인 발주 신청 내역</h3>
				<form name="defaultForm" id="defaultForm" method="get"
					action="order_design_detail_list.do">
					<input type="hidden" name="pageIndex" id="pageIndex"
						value="${ searchVO.pageIndex }" /> <input type="hidden"
						name="searchCondition" id="searchCondition" value="" /> <input
						type="hidden" name="searchKeyword" id="searchKeyword" value="" />
				</form>

				<form name="searchForm" id="searchForm" method="get"
					action="order_design_detail_view.do">
					<input type="hidden" name="seq" id="seq" value="${ view.seq }" /> <input
						type="hidden" name="useq" id="useq" value="${ view.seq }" /> <input
						type="hidden" name="pageIndex" id="pageIndex"
						value="${ searchVO.pageIndex }" /> <input type="hidden"
						name="pageIndex2" id="pageIndex2" value="${ searchVO.pageIndex2 }" />
					<input type="hidden" name="searchCondition" id="searchCondition"
						value="" /> <input type="hidden" name="searchKeyword"
						id="searchKeyword" value="" /> <input type="hidden"
						name="searchConditionTemp" id="searchConditionTemp" value="" /> <input
						type="hidden" name="storename" id="storename" value="" /> <input
						type="hidden" name="name" id="name" value="" />
				</form>

				<form name="writeForm" id="writeForm" method="post"
					enctype="multipart/form-data">
					<input type="hidden" name="cate" value="design" /> <input
						type="hidden" name="seq" id="seq" value="${ view.seq }" />
					<div class="default-cell">
						<h4 class="sub-title">
							디자인 발주 신청 내역 상세<span class="fc01">* 는 필수입력/선택 항목입니다</span>
						</h4>
						...
						<div class="order-list-data">
								...
								<c:choose>
									<c:when test="${ not empty resultList }">
										<c:forEach var="resultList" items="${ resultList }"
											...
											<li>
												<div class="list-img">
													<a href="#;"><img src="${ resultList.filename }"
														alt="제품이미지"
														onerror="this.src='/asset/images/tmp/@img_300x140.jpg'" /></a>
												</div>
												<div class="info">
													<p class="list-name">${ resultList.title }</p>
													<p class="list-tit">
														<span>상품</span><span>가격</span><span>수량<strong
															class="need"></strong></span>
													</p>

													<c:choose>
														<c:when test="${ resultList.priceuse == 'Y' }">

															<c:forEach var="codeResult"
																items="${ fnc:priceDetailList( resultList.seq, view.seq )}"
																varStatus="status">
																<p class="list-data">
																	<span class="data-tit">${ codeResult.options }</span> <span
																		class="data-price"><fmt:formatNumber
																			value="${ codeResult.price}" pattern="#,###" />원</span> <span
																		class="data-num"> <input type="number"
																		name="eacnt" id="ea_${ codeResult.seq}"
																		value="<c:out value="${ codeResult.ea }"/>"
																		data-priceseq="${ codeResult.seq}|${ codeResult.useq}|${ codeResult.price}|${ codeResult.options }"
																		onblur="fnPrice(this);" placeholder="0"
																		style="width: 100px"
																		<c:if test="${ view.status !='O' }">readonly</c:if>
																...
																<c:if test="${ resultList.seq == detailResult.detailseq }">
																		...
																		<p class="list-data">
																			<span class="data-tit">가격은 별도 안내</span> <span
																				...
																				value="<c:out value="${ detailResult.ea }"/>"
													...
													<!-- 									<span class="data-tit">가격은 별도 안내</span> -->
													<!-- 									<span class="data-price">0 원</span> -->
													<%-- 									<span class="data-num"><input type="text" name="eacnt" id="ea_0" value="" data-priceseq="0|${ resultList.seq}|0|${ resultList.title }" onblur="fnPrice(this);" placeholder="0" style="width:100px"  onKeyup="this.value=this.value.replace(/[^0-9]/g,'');" /></span> --%>
													<!-- 								</p> -->
													<%-- 								</c:if> --%>
												</div>
											</li>
							...
							<div>
								<div class="total-list">
									항목 : <span id="txtUseq">${ view.gubun }개</span> / 종류 : <strong
										id="txtSeq">${ view.kind }개</strong>
								</div>
								<div class="total-sum">
									총 합계 금액 : <span id="txtPrice"><fmt:formatNumber
											value="${ view.price}" pattern="#,###" />원</span> <strong>(VAT
										/ 배송료 별도)</strong>
								</div>
								<div class="total-sum">
									최종 출금 예정 금액 : <span id="txtPrice1"><fmt:formatNumber
											value="${ view.price}" pattern="#,###" />원</span>(합계) + <span
										id="txtVat"><fmt:formatNumber value="${ view.vat}"
											pattern="#,###" />원</span>(VAT) + <span id="txtMove"><fmt:formatNumber
											value="${ view.movepay}" pattern="#,###" />원</span>(배송비) : <span
										id="txtTotalPrice"><fmt:formatNumber
											value="${ view.totalprice}" pattern="#,###" />원</span>
								</div>
								...
								<div class="total-data">
									<div class="total-data-area" id="txtOption">
										<p class="tit">
											<span>상품명</span> <span>수량</span> <span>가격</span>
										</p>
									</div>
								</div>
								<!-- //주문리스트 -->
									...
									<tbody>
										...
										<tr style="display: none;">
											...
											<td><input type="text" name="name" style="width: 100%"
												value="<c:out value="${ view.name}"/>"></td>
										</tr>
										<tr style="display: none;">
											...
											<td>
												<div class="input-btns">
													<p class="date-input">
														<input type="text" name="paydt" id="paydt"
															class="datepicker" style="width: 130px;"
															value="<c:out value="${ view.paydt}"/>">
													</p>
												</div>
											</td>
										</tr>
										<tr>
											...
											<td><input type="text" style="width: 100%" name="title"
												id="title" value="<c:out value="${ view.title}"/>">
											</td>
										</tr>
										<tr>
											<th scope="row">내용</th>
											<td><textarea name="conts" id="conts"><c:out
														value="${ view.contents}" /></textarea></td>
										</tr>
										<tr>
											...
											<td>
														...
														<div class="add-file">
															<div class="file-input">
																...
																<input type="text" name="file-name03" dataFile="fileTxt2"
																	...
																	class="read-file" title="업로드된 파일 경로"> <input
																	type="hidden" dataFile="fileOri" name="file3_orinm2"
												...
												</ul> <span class="input-block-info">- 5MB 이하 / jpg, png,
													gif, hwp, txt, ppt, pptx, doc, docx, xls, pdf, zip만 등록 가능</span>
											</td>
										</tr>
										<tr>
											<th scope="row">담당자</th>
											<td><input type="text" style="width: 40%" name="manager"
												id="manager" value="<c:out value="${ view.manager}"/>">
											</td>
										</tr>
													...
													<div class="mulity-check-radio">
														<div>
															<p class="radio-type01">
																<input type="radio" id="radio0101" name="status"
																	value="O"
																	<c:if test="${ view.status == 'O'}">checked</c:if>><label
																	for="radio0101"><span></span>진행중</label>
															</p>
														</div>
														<div>
															<p class="radio-type01">
																<input type="radio" id="radio0102" name="status"
																	value="S"
																	<c:if test="${ view.status == 'S'}">checked</c:if>><label
																	for="radio0102"><span></span>제작완료</label>
															</p>
														</div>
													</div>
												</td>
					...
					<input type="hidden" name="buySeq" id="buySeq" value="" /> <input
						type="hidden" name="buyUseq" id="buyUseq" value="" /> <input
						type="hidden" name="buyPrice" id="buyPrice" value="" /> <input
						type="hidden" name="productseq" id="productseq" value="" /> <input
						type="hidden" name="detailseq" id="detailseq" value="" /> <input
						type="hidden" name="ea" id="ea" value="" /> <input type="hidden"
						name="gubun" id="gubun" value="" /> <input type="hidden"
						name="kind" id="kind" value="" /> <input type="hidden"
						name="totalprice" id="totalprice" value="0" /> <input
						type="hidden" name="orderseq" id="orderseq" value="" /> <input
						type="hidden" name="vat" id="vat" value="" /> <input type="hidden"
						name="movepay" id="movepay" value="" /> <input type="hidden"
				...
				<div class="default-cell">
					...
					<form name="replyForm" id="replyForm" method="post"
						...
						<input type="hidden" name="pageIndex2"
							value="${ searchVO.pageIndex2 }" /> <input type="hidden"
							name="title" id="title" value="-" /> <input type="hidden"
							name="useyn" id="useyn" value="Y" />
						<div class="table-list-data">
							<fieldset>
								...
								<table class="view">
									...
									<tbody>
										...
										<tr>
											...
											<td>
												<div class="file-input">
													<input type="hidden" name="file01_del" id="file01_del"
														value="" /> <input type="file" name="file01" id="f01"
														class="input-file" onchange="front.form.inputFile(this)"
														title="파일 업로드 찾기"> <input type="text"
														name="file-name01" id="fn01" readonly="" placeholder=""
														class="read-file" title="업로드된 파일 경로"> <a
														href="javascript:;" class="btn-close"
														onclick="front.form.deleteFileText(this,'file01');">파일삭제</a>
													<a href="javascript:;" class="btn-01 type-03 round">찾아보기</a>
												</div> <span class="input-block-info">- 5MB 이하 / jpg, png,
													gif zip 만 등록 가능</span>
											</td>
										</tr>
										<tr>
											<th scope="row">작성</th>
					...
					<div class="cbox-write-list">
						<div class="table-sort">
							전체 : <span><fmt:formatNumber
									value="${ paginationInfo.totalRecordCount}" pattern="#,###" /></span>건
						</div>
						<ul>
							<c:choose>
								<c:when test="${ not empty replyList }">
									<c:forEach items="${ replyList }" var="result"
										varStatus="status">

										<li>
											<div class="cbox-txt">
												${ result.contents }
												<fmt:parseDate value="${ result.regdt }" var="regDt"
													pattern='yyyy-MM-dd HH:mm:ss' />
												<fmt:formatDate value="${ regDt }" pattern='yyyy.MM.dd'
													var="regDttmp" />
												<fmt:formatDate value="${ regDt }" pattern='HH:mm:ss'
													var="regTime" />

												<fmt:parseDate value="${ result.regdt }" var="chg_dttm"
													pattern="yyyy-MM-dd" />
												<fmt:parseNumber
													value="${ chg_dttm.time / (1000*60*60*24) }"
													integerOnly="true" var="chgDttm"></fmt:parseNumber>
												<c:if test="${ today - chgDttm le 7 }">
													<span class="ico-new"><img
														src="/asset/images/icon/icon_new.png" alt="New"></span>
												</c:if>
												<a href="#" class="btn-02 type-02 "
													onclick="deleteRp('${ result.seq }')">삭제</a>
											</div>
											<div class="cbox-date">
												<strong class="cbox-user"><a href="#">${ result.regid }</a></strong><strong><c:out
														value="${ regDttmp }" /></strong><span>(<c:out
														value="${ regTime }" />)
												</span></strong>
											</div>
											<ul class="cbox-file">
												<c:if test="${ result.filename != '' }">
													<li><a
														href="/download.do?path=${ result.filename }&fileName=${ result.orgfilename }">${ result.orgfilename }</a></li>
												</c:if>
											</ul> <!-- <div class="cbox-btn"><a href="#" class="btn-01 type-03 round">삭제</a></div> -->
										</li>
			...
			<!-- dFoot -->
			<footer id="dFoot">
				<div class="copy">Copyright ⓒ 2018 DAILYKING INC. All rights
					reserved.</div>
			</footer>
			<!-- //dFoot -->
		</section>
	...
	<script>
		...
		function fnPrice(obj) {
			...
			$("#txtOption")
					.html(
							"<p class='tit'><span>상품명</span><span>수량</span><span>가격</span></p>");
			...
			for (i = 0; i < arrev.length; i++) {
				...
				if (arrev[i].value != '') {
					...
					var p = $("input[name='eacnt']").eq(i).data("priceseq")
							.toString();
					...
					if (parseInt(e) > 0) {
						...
						$("#txtOption").html(
								$("#txtOption").html()
										+ "<p class='data'><span class='tit'>"
										+ options
										+ " </span><span class='num'> " + e
						...
						for (k = 0; k < buyUseqArr.length; k++) {
							if (buyUseqArr[k] == useq
									&& txtUseqTmpArr.indexOf(useq) < 0) {
		...
		$("._addBtn")
				.click(
						function() {
							...
							if ($("input[name=file-name01]").val() != ''
									&& $("input[name=file-name01]").val() != undefined) {
								var fileName = $("input[name=file-name01]")
										.val();
							...
							var fnApplyCallback = function(objResponse,
									statusText, xhr, $from) {
		...
		$("._replyBtn")
				.click(
						function() {
							...
							if (frm.find("input[name=file-name01]").val() != ''
									&& frm.find("input[name=file-name01]").val() != undefined) {
								var fileName = frm.find("input[name=file-name01]")
										.val();
		...
		function deleteRp(ob) {
			...
			$.ajax({
				url : "./design_Reply_delete_proc.do",
				type : "POST",
				data : {
					'orgseq' : ob
				},
