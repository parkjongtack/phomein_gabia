        ...
        var fnApplyCallback = function(objResponse, statusText, xhr, $from){
        	...
        	if (statusText == "success"){
                if( obj["result"] == 'S' ){
                	...
                	//<![CDATA[
               		<c:forEach var="codeResult" items="${fnc:menuList('')}" varStatus="status">
               		<c:forEach var="subResult" items="${fnc:menuListSub( codeResult.menuCd )}" varStatus="status">
               		if ( menuCd == '${subResult.menuCd}' && '${subResult.menuCd}' != '999' ){
               			location.replace("${subResult.link}");
               		}
               		</c:forEach>
               		</c:forEach>
                	//]]>
