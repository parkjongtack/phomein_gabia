...
<div id="wrap">
	...
	<section id="dBody">
		...
		<section id="contents">
			...
			<!-- //location -->
			<h3 class="con-title">팝업 관리</h3>

			<form name="searchForm" id="searchForm" action="popup_list.do">
				<input type="hidden" id="pageIndex" name="pageIndex" value="${paginationInfo.currentPageNo }"/>
				<div class="table-list-data">
					<fieldset>
						<legend>팝업 관리 검색 입력 폼</legend>
						<table class="view">
							<caption>팝업 관리 검색 기본정보 입력 폼 테이블</caption>
							<colgroup>
							...
							<tbody>
								<tr>
									...
									<td>
										<div class="mulity-check-radio">
											...
											<div><p class="radio-type01"><input type="radio" id="radio0102" name="search_useyn" value="1" <c:if test="${search_useyn == 1}">checked</c:if>><label for="radio0102"><span></span>활성</label></p></div>
											<div><p class="radio-type01"><input type="radio" id="radio0103" name="search_useyn" value="2" <c:if test="${search_useyn == 2}">checked</c:if>><label for="radio0103"><span></span>비활성</label></p></div>
										</div>
									</td>
								</tr>
								<tr>
									<th scope="row">노출범위</th>
									<td>
										<div class="mulity-check-radio">
											<div><p class="radio-type01"><input type="radio" id="radio0201" name="search_cate" value="" <c:if test="${empty search_cate}">checked</c:if>><label for="radio0201"><span></span>전체</label></p></div>
								...
								<tr>
									<th scope="row">제목</th>
									<td>
										<input type="text" name="search_title" id="search_title" value="${search_title }" placeholder="제목을 입력하세요" style="width:100%">
									</td>
								</tr>
							</tbody>
						</table>
						...
						<c:choose>
							...
							<c:forEach items="${resultList }" var="result" varStatus="status" >
								<tr>
									...
									<td><c:forEach var="codeResult" items="${ fnc:codeListSub('115') }" varStatus="status">
				                        	<c:if test="${result.cate == codeResult.code}"><c:out value="${codeResult.name}"/></c:if>
				                        </c:forEach>
				                    </td>
									<td class="al-l"><a href="javascript:;" name="view_${result.seq }" data-seq="${result.seq }" data-type="udt" data-page="${pageIndex }" data-search_useyn="${search_useyn }" data-search_cate="${search_cate }" data-search_title="${search_title }" class="btn-link">${result.title }</a></td>
									<td>
										<p class="table-date">
										<fmt:parseDate value="${result.startdt}" var="startdt" pattern='yyyy-MM-dd HH:mm:ss'/>
										<fmt:formatDate value="${startdt}" pattern='yyyy.MM.dd' var="startDttmp"/>
										<fmt:formatDate value="${startdt}" pattern='HH:mm:ss' var="startTime"/>
										<c:out value="${startDttmp }" />
										<span>(<c:out value="${startTime }"/>)</span></p>
										<p class="table-date">
										<fmt:parseDate value="${result.enddt}" var="enddt" pattern='yyyy-MM-dd HH:mm:ss'/>
										<fmt:formatDate value="${enddt}" pattern='yyyy.MM.dd' var="endDttmp"/>
										<fmt:formatDate value="${enddt}" pattern='HH:mm:ss' var="endTime"/>
										~ <c:out value="${endDttmp }" />
										<span>(<c:out value="${endTime }"/>)</span></p>
									</td>
									<td>
										<p class="table-date">
									...
									<td>
										<c:choose>
											<c:when test="${result.useyn == 'Y'}">
												<span class="fc03">[활성]</span>
											</c:when>
											<c:otherwise>
												<span class="fc01">[비활성]</span>
											</c:otherwise>
										</c:choose>
									</td>
									<td>
										<div class="table-btns row">
											<p><a href="javascript:;" name="view_${result.seq }" data-seq="${result.seq }" data-type="udt" data-page="${pageIndex }" data-search_useyn="${search_useyn }" data-search_cate="${search_cate }" data-search_title="${search_title }" class="btn-01 type-02 round">수정</a></p>
							...
							<tr>
								...
								<td>9</td>
								<td>기업사이트</td>
								<td class="al-l"><a href="#" class="btn-link">10x스노우 콜라보레이션 이벤트</a></td>
								<td><p class="table-date">2018.12.10 <span>(23:00:00)</span></p><p class="table-date">~ 2018.12.10 <span>(23:00:00)</span></p></td>
								<td><p class="table-date">2018.12.10<br><span>(23:00:00)</span></p></td>
								<td><span class="fc01">비활성</span></td>
								<td>
									<div class="table-btns row">
...
$(function(){
	function updateUseYn(useyn, callback) {
		...
		$.ajax({
            url: "/admin/site/update_useyn_popup.do",
            type: "POST",
            data: {
	...
	function callbackFunc() {
        ...
        form.setAttribute("action", "/admin/site/popup_list.do");
        ...
        hiddenField.setAttribute("value", $("input[name='search_cate']:checked").val());
        form.appendChild(hiddenField);
        hiddenField = document.createElement("input");
        hiddenField.setAttribute("type", "hidden");
	...
	$("a[name^='view_']").on("click", function(){
        var form = document.createElement("form");
        form.setAttribute("action", "/admin/site/popup_view.do");
        var hiddenField = document.createElement("input");
        hiddenField.setAttribute("type", "hidden");
	...
	$("a[id^='delete_']").on("click", function(){
        ...
        if (confirm('항목을 삭제하시겠습니까?')) {
	        $.ajax({
	            url: "/admin/site/popup_delete.do",
	            type: "POST",
	            data: {'seq' : seq},
	            dataType: 'json',
	            cache: false,
	...
	$("#resetBtn").on("click", function(){
		...
		location.replace("./popup_list.do");
	});
