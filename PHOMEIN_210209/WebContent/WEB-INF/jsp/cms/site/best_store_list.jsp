...
<div id="wrap">
	...
	<section id="dBody">
		...
		<section id="contents">
			...
			<!-- //location -->
			<h3 class="con-title">우수매장 인터뷰 관리</h3>

			<form name="searchForm" id="searchForm" action="best_store_list.do">
				<input type="hidden" id="pageIndex" name="pageIndex" value="${paginationInfo.currentPageNo }"/>
				<div class="table-list-data">
					<fieldset>
						<legend>우수매장 인터뷰 검색 입력 폼</legend>
						<table class="view">
							<caption>우수매장 인터뷰 검색 기본정보 입력 폼 테이블</caption>
							<colgroup>
							...
							<tbody>
								...
								<tr>
									<th scope="row">매장명</th>
									<td>
										<input type="text" name="search_store" id="search_store" value="${search_store }" placeholder="매장명을 입력하세요" style="width:100%">
									</td>
								</tr>
							</tbody>
						</table>
					...
					<table class="list">
						...
						<c:choose>
							...
							<tr>
								...
								<td class="al-l"><a href="javascript:;" name="view_${result.seq }" data-seq="${result.seq }" data-type="udt" data-page="${pageIndex }" data-search_useyn="${search_useyn }" data-search_store="${search_store }" class="btn-link">${result.store }</a></td>
								<td><a href="javascript:;" class="btn-link">${result.title }</a></td>
								<td>
									<p class="table-date">
								...
								<td>
									<div class="table-btns row">
										<p><a href="javascript:;" name="view_${result.seq }" data-seq="${result.seq }" data-type="udt" data-page="${pageIndex }" data-search_useyn="${search_useyn }" data-search_store="${search_store }" class="btn-01 type-02 round">수정</a></p>
							...
							<tr>
								...
								<td>9</td>
								<td class="al-l"><a href="#" class="btn-link">잠실점</a></td>
								<td>홍길동</td>
							...
							<tr>
								...
								<td>8</td>
								<td class="al-l"><a href="#" class="btn-link">원주혁신도시점</a></td>
								<td>송창근 점주</td>
								<td><p class="table-date">2018.12.10<br><span>(23:00:00)</span></p></td>
								<td><p class="table-date">2018.12.10<br><span>(23:00:00)</span></p></td>
...
$(function(){
	function updateUseYn(useyn, callback) {
		...
		$.ajax({
            url: "/admin/site/update_useyn_best_store.do",
            type: "POST",
            data: {
	...
	function callbackFunc() {
        ...
        form.setAttribute("action", "/admin/site/best_store_list.do");
        ...
        hiddenField = document.createElement("input");
        hiddenField.setAttribute("type", "hidden");
        hiddenField.setAttribute("name", "search_store");
        hiddenField.setAttribute("value", $("input[name='search_store']").val());
        form.appendChild(hiddenField);
	...
	$("a[name^='view_']").on("click", function(){
        var form = document.createElement("form");
        form.setAttribute("action", "/admin/site/best_store_view.do");
        var hiddenField = document.createElement("input");
        hiddenField.setAttribute("type", "hidden");
        ...
        hiddenField.setAttribute("type", "hidden");
        hiddenField.setAttribute("name", "search_store");
        hiddenField.setAttribute("value", !!$(this).data("search_store") ? $(this).data("search_store") : '');
        form.appendChild(hiddenField);
        document.body.appendChild(form);
        form.submit();
	});
	...
	$("a[id^='delete_']").on("click", function(){
        ...
        if (confirm('항목을 삭제하시겠습니까?')) {
	        $.ajax({
	            url: "/admin/site/best_store_delete.do",
	            type: "POST",
	            data: {'seq' : seq},
	            dataType: 'json',
	            cache: false,
	...
	$("#resetBtn").on("click", function(){
		...
		$("input[name='search_store']").val("");
		location.replace("./best_store_list.do");
	});
