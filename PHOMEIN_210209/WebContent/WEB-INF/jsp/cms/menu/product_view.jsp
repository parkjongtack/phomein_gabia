						...
						<fieldset>
							...
							<table class="view">
								...
								<tbody>
									...
									<tr>
										<th scope="row">설정<span class=""></span></th>
										<td>
											<div class="mulity-check-radio">
									...
									<tr>
										<th scope="row">제품이미지<br><em class="fc01">(사이즈 x 사이즈)</em><span class="need"></span></th>
										<td>
...
$(function(){
	...
	var submitFunction = function (form) {
	    ...
	    $.ajax({
            url: "/admin/menu/product_modify.do",
            type: "POST",
            data: formdata,
            ...
            success: function (data) {
            	...
            	if(!!data.result) {
            		if(data.result == 'SUCCESS') {
            			alert('정상적으로 등록되었습니다.');
            			location.href="/admin/menu/product_list.do";
            		}
            	}
	...
	$("#listBtn").on("click", function(){
		location.href='product_list.do?seq=' + $("#seq").val() + '&pageIndex=' + $("#pageIndex").val() + '&search_useyn=' + $("#search_useyn").val() + '&search_site_ck=' + $("#search_site_ck").val() + '&search_name=' + $("#search_name").val();
	});
	...
	$("#checkPcodeBtn").on("click", function() {
		...
		if(!!$pcode) {
			$.ajax({
	            url: "/admin/menu/product_check.do",
	            type: "POST",
	            data: {'pcode' : $pcode},
	            dataType: 'json',
	            success: function (data) {
