...
<div id="wrap">
	...
	<section id="dBody">
		...
		<section id="contents">
			...
			<!-- //location -->
			<h3 class="con-title">제품관리</h3>

			<form name="searchForm" id="searchForm" action="product_list.do">
...
$(function(){	
	function updateUseYn(useyn, callback) {
		...
		$.ajax({
            url: "/admin/menu/update_useyn_product.do",
            type: "POST",
            data: {
	...
	function callbackFunc() {
        ...
        form.setAttribute("action", "/admin/menu/product_list.do");
	...
	$("a[name^='view_']").on("click", function(){
        var form = document.createElement("form");
        form.setAttribute("action", "/admin/menu/product_view.do");
        var hiddenField = document.createElement("input");
        hiddenField.setAttribute("type", "hidden");
	...
	$("a[id^='delete_']").on("click", function(){
        ...
        if (confirm('항목을 삭제하시겠습니까?')) {
	        $.ajax({
	            url: "/admin/menu/product_delete.do",
	            type: "POST",
	            data: {'seq' : seq},
	            dataType: 'json',
	            cache: false,
	...
	$("#resetBtn").on("click", function(){
		...
		location.replace("./product_list.do");
	});
	...
	$("#excelDownBtn").on("click", function(){
		location.href='/admin/menu/excelDown.do?pageIndex=' + $(this).data("page") + '&search_useyn=' + $(this).data("search_useyn") + '&search_site_ck=' + $(this).data("search_site_ck") + '&search_name=' + $(this).data("search_name");
	});
