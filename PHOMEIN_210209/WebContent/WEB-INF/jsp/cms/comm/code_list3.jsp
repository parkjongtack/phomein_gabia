...
<html lang="ko">
<head>
<title>코드관리 | 공통관리 | 포메인 통합관리자</title>
...
<div id="wrap">
	...
	<section id="dBody">
		...
		<section id="contents">
			...
			<!-- //location -->
			<h3 class="con-title">코드 관리 / 소분류</h3>

			<form name="searchForm" id="searchForm" action="code_list3.do">
				...
				<input type="hidden" id="code_type" name="code_type" value="sub_sub_code"/>
			</form>
					...
					<fieldset>
						...
						<table class="view">
							...
							<tbody>
								...
								<tr>
									<th scope="row">소분류코드 <span class="need"></span></th>
									<td>
										<input type="text" name="sub_sub_code" id="sub_sub_code" maxlength="9" placeholder="소분류코드를 입력하세요" style="width:100%">
									</td>
								</tr>
								<tr>
									<th scope="row">소분류명 <span class="need"></span></th>
									<td>
										<input type="text" name="sub_sub_code_name" id="sub_sub_code_name" placeholder="소분류명을 입력하세요" style="width:100%">
									</td>
								</tr>
								<tr>
					...
					<table class="list" id="code_table">
						...
						<c:choose>
							...
							<tr>
								...
								<td>${result.pcode }</td>
								<td class="al-l"><a href="#" class="btn-link">${result.pname }</a></td>
								<td>${result.code }</td>
							...
							<tr>
								...
								<td>0001</td>
								<td class="al-l"><a href="#" class="btn-link">소분류명02</a></td>
								<td>
									<div class="table-btns row">
							...
							<tr>
								...
								<td>0001</td>
								<td class="al-l"><a href="#" class="btn-link">소분류명01</a></td>
								<td>
									<div class="table-btns row">
										<p><a href="#" class="btn-up">위로</a></p>
										<p><a href="#" class="btn-down">아래로</a></p>
...
<script>
$(function(){
	$("#sub_sub_code").val($("input[name='nextcode']").val());
	...
	var message = {
		...
		sub_sub_code: {
	        required: '소분류코드를 입력하세요',
	        maxlength: '소분류코드는 9자리 입니다'
	    },
	    sub_sub_code_name: {
	        required: '소분류명을 입력하세요'
	    }
	};
	var submitFunction = function () {
		if($("#sub_sub_code").val().substring(0, 6) != $("#pcode").val()) {
    	...
    	var sub_sub_code = $("#sub_sub_code").val();
    	var sub_sub_code_name = $("#sub_sub_code_name").val();
    	var code_type = 'sub_sub_code';
    	var type = $("#type").val();
	    ...
	    $.ajax({
            ...
            data: {
            	'seq' : seq,
            	'useyn' : useyn,
            	'sub_sub_code' : sub_sub_code,
            	'sub_sub_code_name' : sub_sub_code_name,
            	'code_type' : code_type,
            	'type' : type,
            	'ordr' : ordr
            },
            ...
            success: function (data) {
            	...
            	if(!!data.result) {
            		if(data.result == 'SUCCESS') {
            			alert('정상적으로 등록되었습니다.');
            			location.href="/admin/comm/code_list3.do?useyn=" + useyn + "&code=" + sub_sub_code + "&code_name=" + sub_sub_code_name + "&code_type=sub_sub_code&type=udt";
            		} else {
	...
	$("#resetBtn").on("click", function(){
		...
		$("input[name='sub_sub_code']").val("");
		$("input[name='sub_sub_code_name']").val("");
	...
	$(".modifyBtn").on("click", function(){
		...
		$("input[name='sub_sub_code']").val($(this).data("code"));
		$("input[name='sub_sub_code_name']").val($(this).data("code_name"));
