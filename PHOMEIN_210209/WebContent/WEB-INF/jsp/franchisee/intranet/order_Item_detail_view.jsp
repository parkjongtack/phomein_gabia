...
<html lang="ko">
<head>
<title>본사 물품 신청 내역 상세/수정/답글 | 인트라넷 | 포메인 가맹점 관리</title>
	...
	<section id="dBody">
		...
		<section id="contents">
			...
			<div class="default-cell">
				<h4 class="sub-title">본사 물품 신청 내역 상세/수정<span class="fc01">* 는 필수입력/선택 항목입니다</span></h4>
				...
				<%-- <div class="table-list-data">
					<table class="list large">
						...
						<c:choose>
							...
							<tr>
								<td>
									<span class="list-img"><img src="${resultList.filename }" alt="제품이미지" onerror="this.src='/asset/images/tmp/@img_300x140.jpg'" /></span>
								</td>
								<td>${resultList.title }</td>
								<td>${resultList.contents }</td>
								...
								<td>
									<ul class="order-list">
										<c:forEach var="codeResult" items="${fnc:priceDetailList( resultList.seq, view.seq )}"  varStatus="status">
										<li>
											<input type="text" name="eacnt" id="ea_${codeResult.seq}" value="<c:out value="${codeResult.ea }"/>" data-priceseq="${codeResult.seq}|${codeResult.useq}|${codeResult.price}|${resultList.title } ${codeResult.options }" 
												onblur="fnPrice(this);" placeholder="0" style="width:100px" <c:if test="${view.status !='O' }">readonly</c:if> />
										</li>
										</c:forEach>
									</ul>
								</td>
					...
					<fieldset>
						...
						<table class="view">
							...
							<tbody>
								...
								<c:if test="${view.contents != ''}">
								<tr style="display:none;">
									<th scope="row">내용</th>
									<td>
										<textarea name="conts" id="conts">${view.contents}</textarea>
									</td>
								</tr>
								</c:if>
							</tbody>
