...
<html lang="ko">
<head>
<title>디자인 발주 신청 등록 | 인트라넷 | 포메인 가맹점 관리</title>
	...
	<section id="dBody">
		...
		<section id="contents">
			...
			<div class="default-cell">
				...
				<input type="hidden" name="cate" id="cate" value="design"/>
				<div class="line-info-list">
					<ul>
						<li>발주 하실 품목을 선택하신 후, 상세 내용란에 홍보물 진행안(항목, 내용, 수량 등)을 정확히 작성해주셔야 작업이 진행됩니다.</li>
						<li>접수 순서에 따라 순차적으로 디자인 작업이 진행되며, 접수일로부터 물품 수령까지 최대 2주가 소요됩니다.</li>
						<li>결제는 출금 전 결제 내역 문자 안내 후 CMS 자동 출금됩니다. (입금 시 별도 연락 요망. Tel. 1588-2422 대표번호(연결 3번)</li>
						<li>총 결제 금액이 30,000원 미만일 경우, 택배 발송비 3,000원이 추가됩니다.</li>
					</ul>
				</div>

				<div class="privacy-box">
					<h3>개인정보의 제공 및 활용 동의서</h3>

					<div class="rules-box">
						<div class="rules-txt">
							<dl>
								<dt>제 1 조 (목적)</dt>
								<dd>본 이용약관(이하 '약관')은 주식회사 데일리킹(이하 '본사') 10 홈페이지 인트라넷 이용 고객(이하 '가맹점')에게 회사가 제공하는 홍보,판촉물 시안물 제작 서비스 이용에 관한 제반 사항 및 기타 필요한 사항을 구체적으로 규정함을 목적으로 한다.</dd>
							</dl>

							<dl>
								<dt>제 2 조 (권한의 위임)</dt>
								<dd>가맹점은 홍보,판촉물 제작에 관련된 모든 권한을 본사에게 위임한다.</dd>
							</dl>
							...
							<dl>
								...
								<dd>
									<ol>
										<li>xxx 홈페이지 회원은 "본인회원"만을 원칙으로 합니다.</li>
										<li>xxx 홈페이지 회원은 가입 및 이용이 무료입니다.</li>
										<li>"본인회원"이란 이 규약을 승인하고 xxx 홈페이지 서비스의 회원신청 양식에 의거, 본인의 신상정보를 사실에 입각, 입력하여 ID와 Password를 발급받으신 분을 의미합니다.(법인 혹은 단체회원으로 가입신청을 하실 수는 없습니다.)</li>
										<li>회원은 3항의 회원정보 기재내용에 변경이 발생한 경우, 즉시 변경사항을 기재하여야 합니다.</li>
										<li>xxx 홈페이지 서비스 제공은 다음 사항에 해당하는 경우에는 가입승인을 유보 또는 거부할 수 있습니다.<br>
										1) 가명으로 회원에 가입할 경우.<br>
										2) 혐오스럽거나 타인에게 불쾌감을 줄 수 있는 아이디로 회원가입을 할 경우.<br>
										3) 다른 사람의 명의를 빌리거나 도용하여 차명으로 신청하였을 경우.<br>
										4) 기타 가입신청 항목에 허위로 기재할 경우.<br>
										5) 관계법령에 위배되거나 미풍양속을 저해하고자 하는 목적으로 회원가입신청을 할 경우.</li>
									</ol>
								</dd>
							</dl>
						</div>
					...
					<div class="mt20 al-r">
						<p class="check-type01"><input type="checkbox" id="c02" name="agree" value="Y"><label for="c02"><span></span>개인정보 수집 및 이용에 동의합니다. <strong class="fc01">(필수)</strong></label></p>
					</div>
				</div>
				...
				<div class="table-list-data">
					<fieldset>
						<legend>디자인 발주 신청 등록 기본정보 입력 폼</legend>
						<table class="view">
							<caption>디자인 발주 신청 등록 기본정보 입력 폼 테이블</caption>
							<colgroup>
							...
							<tbody>
								<tr style="display:none;">
									...
									<td>
										<input type="hidden" name="name" id="name" value="${storeSession.name }" style="width:100%" placeholder="입금자명 입력하세요">
									</td>
								</tr>
								<tr style="display:none;">
...
<!-- 											<input type="hidden" name="orgname" value=""/> -->
<%-- 											<input type="file" dataFile="file" name="file01" id="f01" class="input-file" value="${fileresult.filename}" onchange="front.form.inputFile(this)" title="파일 업로드 찾기"> --%>
<%-- 											<input type="text" dataFile="fileTxt" name="file-name01" id="fn01" readonly="" value="${fileresult.filename}" placeholder="" class="read-file" title="업로드된 파일 경로"> --%>
<!-- 											<a href="javascript:void(0);" class="btn-close" onclick="front.form.deleteFileText(this,'file01');front.form.deleteInputFile(this);">파일삭제</a> -->
<!-- 											<a href="javascript:;" class="btn-01 type-03 round">찾아보기</a> -->
<%-- 											<c:if test="${not empty fileresult.filename}"> --%>
<%-- 											<div class="thumb"><img src="${fileresult.filename}" ></div> --%>
<%-- 											</c:if> --%>
