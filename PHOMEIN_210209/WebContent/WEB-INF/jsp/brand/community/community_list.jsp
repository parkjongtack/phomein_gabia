...
<div id="wrap">
	...
	<section id="dBody" class="community">
		...
		<c:if test="${searchVO.boardType == 'sns'}">
			<!-- page-title -->
			<div class="page-title">
				<div class="visual open" style="background-image:url('/brand/images/content/sub_visual_04.jpg');"></div>
		...
		<div id="contents" class="content">
			<div class="inner">
				<!-- page-tab -->
				<div class="page-tab">
					<form id="searchForm" name="searchForm" method="post" action="/brand/community/${searchVO.boardType}/community_list.do">
					...
					<c:if test="${searchVO.boardType == 'notice'}">
						<ul>
							<li <c:if test="${searchVO.cate == ''}"> class=actived </c:if> ><a href="/brand/community/notice/community_list.do">전체</a></li>
							<c:forEach var="codeResult" items="${ fnc:codeListSub('107') }" varStatus="status">
								<c:if test="${codeResult.code != '107202'}">
	                      			<li <c:if test="${searchVO.cate == codeResult.code}"> class='actived' </c:if> ><a href="/brand/community/notice/community_list.do?cate=${codeResult.code}"><c:out value="${codeResult.name}"/></a></li>
	                      		</c:if>
	                      	</c:forEach>
						</ul>
					</c:if>
					<c:if test="${searchVO.boardType == 'event'}">
						<ul>
							<li <c:if test="${searchVO.event_progress == ''}"> class=actived </c:if> ><a href="/brand/community/event/community_list.do">전체</a></li>
							<li <c:if test="${searchVO.event_progress == 'P'}"> class='actived' </c:if> ><a href="/brand/community/event/community_list.do?event_progress=P">진행중 이벤트</a></li>
							<li <c:if test="${searchVO.event_progress == 'E'}"> class='actived' </c:if> ><a href="/brand/community/event/community_list.do?event_progress=E">종료된 이벤트</a></li>
						</ul>
					</c:if>
					<c:if test="${searchVO.boardType == 'sns'}">
						<ul>
								...
								<c:choose>
									<c:when test="${not empty resultList }" >
										<c:forEach items="${resultList }" var="result" varStatus="status" >
											<li>
												<p class="img"><a href="javascript:void(0);" onclick="javascript:fnDetailView('/brand/community/${searchVO.boardType}/community_view.do', <c:out value="${result.seq }" />); return false;"><img src="${result.pc_thimg }" alt="${result.pc_alt}"/></a></p>
												...
												<p class="tit"><a href="javascript:void(0);" onclick="javascript:fnDetailView('/brand/community/${searchVO.boardType}/community_view.do', <c:out value="${result.seq }" />); return false;">
							...
							<ul id="eventUl">
								<c:choose>
									<c:when test="${not empty resultList }" >
										<c:forEach items="${resultList }" var="result" varStatus="status" >
											<li>
												<p class="img"><a href="javascript:void(0);" onclick="javascript:fnDetailView('/brand/community/${searchVO.boardType}/community_view.do', <c:out value="${result.seq }" />); return false;"><img alt="" src="${result.pc_thimg }"></a></p>
												<p class="ico-stamp">
												<span>
												...
												<c:if test="${result.event_progress == 'P' }"><p class="ico-proceed"><span>진행중</span></p></c:if>
												<c:if test="${result.event_progress == 'E' }"><p class="ico-closed"><span>마감</span></p></c:if>
												<p class="tit"><a href="javascript:void(0);" onclick="javascript:fnDetailView('/brand/community/${searchVO.boardType}/community_view.do', <c:out value="${result.seq }" />); return false;">
												${fnc:xssContents(result.title )} <c:if test="${result.newYn == 'Y'}"><span class="new">new</span></c:if></a></p>
												<p class="txt"><c:out value="${result.stitle }"/></p>
					...
					<div class="sns-list">
						...
						<c:if test="${ currentPage lt lastPage  }">
							<div class="btn-box mt80" id="snsMoreDiv">
								<a class="btn-02 type-03 btn-plus" href="javascript:void(0);" onclick="selectSnsMore();">더보기 +</a>
							</div>
						</c:if>
						<!-- //btn-box -->
					</div>
