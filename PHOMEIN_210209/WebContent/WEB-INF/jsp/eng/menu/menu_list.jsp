...
<!-- 			<span class="txt">특별하지 않은 포메인은 없습니다. 조금 더 특별한 포메인을 소개합니다.</span> -->
		...
		<div id="contents" class="content">
			<div class="inner">
				<div class="page-tab">
					<ul>
               			<li class="actived"><a href="/eng/menu/menu_list.do">phomain</a></li>
               			<li><a href="/eng/menu/red_menu_list.do">phomain red</a></li>
					</ul>
				</div>
				<div class="search-option">
					...
					<div class="option">
						<c:forEach items="${encodes }" var="result" varStatus="status">
								...
								<c:forEach items="${resultList }" var="result" varStatus="status" >
									<li>
										<a href="javascript:;" onclick="openLayer(${result.seq});">
											...
											<div class="name">
												<p class="ko-name">${result.en_name }</p>
											</div>
											<div class="info">
												<p class="ko-name">${result.en_name }</p>
												<p class="txt">${result.en_menu }</p>
											</div>
										</a>
									</li>
								</c:forEach>
								<li class="tackout">
									<div class="img"><img src="../images/menu/tackout_service_1000x1000.png" alt=""/></div>
									<div class="name">
...
<script type="text/javascript">
	...
	function openLayer(seq) {
		$.ajax({
	     	url: "/eng/menu/get_menu_view_ajax.do",
	        type: "POST",
	        data: {
	        	'seq' : seq
	        },
	        ...
	        success: function (data) {
	    	    ...
	    	    tmplVal.encodes = data.encodes;
	    	    tmplVal.resultView = data.resultView;
	    	    tmplVal.enmenuList = data.enmenuList;
	...
	$("#reset_btn").on('click',function(){
		...
		var option = {
		     	url: "/brand/menu/get_menu_list_ajax.do",
		        type: "POST",
		        data: {
		        	'best_ck' : 'Y',
		        	'hot_ck' : 'Y',
	...
	function makeMenuList(option) {
		...
		if (option === null || option === undefined) {
			...
			option = {
			     	url: "/eng/menu/get_menu_list_ajax.do",
			        type: "POST",
			        data: {
			        	'best_ck' : best_ck,
			        	'hot_ck' : hot_ck,
...
<script id="menuListTmpl" type="text/x-tmpl">
	...
	{{each(key,item) jsonArr}}
		<li>
			<a href="javascript:;" onclick="openLayer('{{= item.seq}}');">
				...
				<div class="name">
					<p class="ko-name">{{= item.en_name}}</p>
				</div>
				<div class="info">
					<p class="ko-name">{{= item.en_name}}</p>
					<p class="txt">{{= item.en_menu}}</p>
				</div>
			</a>
		</li>
	{{/each}}
	<li class="tackout">
		<div class="img"><img src="../images/menu/tackout_service_1000x1000.png" alt=""/></div>
		<div class="name">
			<p class="ko-name">TACKOUT SERVICE</p>
		</div>
	</li>
	{{/if}}
	{{if jsonArr.length == 0}}
...
<script id="menuViewTmpl" type="text/x-tmpl">
	<div class="menu-tap">
		<ul>
			{{each(key,item) encodes}}
				<li class="{{if item.code == resultView.en_code}}actived{{/if}}"><a {{if item.enmenucode != ''}}onclick="openLayer('{{= item.enmenucode}}');"{{/if}}>{{= item.name}}</a></li>
			{{/each}}
		</ul>
	...
	<div class="view-con">
		...
		<div class="swiper-container">
			<div class="swiper-wrapper">
				{{each(key,item) enmenuList}}
				<div class="swiper-slide" data-en_name="{{= item.en_name}}" data-en_menu="{{= item.en_menu}}" data-s_price="{{= item.s_price}}" data-l_price="{{= item.l_price}}" data-c_price="{{= item.c_price}}" data-c_yn="{{= item.c_yn}}">
					<div class="img">
           		    	<img src="{{= item.f_file1}}">
					</div>
					<div class="info">{{= item.en_name}}</div>
				</div>
				{{/each}}
			</div>
		...
		<div class="menu-info">
			<div class="info">
				<div class="name">
					...
					<p class="txt en-menu">{{= resultView.en_menu}}</p>
				</div>
<!--
				<div class="price">
					...
					{{/if}}
					<p class="txt">* Prices may vary by store</p>
				</div>
-->
			</div>
		</div>
