	...
	<section id="dBody" class="community">
		...
		<c:if test="${searchVO.boardType == 'notice'}">
			...
			<div class="page-title">
				<div class="visual" style="background-image:url('/eng/images/content/sub_visual_04.jpg');"></div>
		...
		<c:if test="${searchVO.boardType == 'sns'}">
			<!-- page-title -->
			<div class="page-title">
				<div class="visual open" style="background-image:url('/eng/images/content/sub_visual_04.jpg');"></div>
		...
		<div id="contents" class="content">
			<div class="inner">
				<!-- page-tab -->
				<div class="page-tab">
					<form id="searchForm" name="searchForm" method="post" action="/eng/community/${searchVO.boardType}/community_list.do">
					...
					<c:if test="${searchVO.boardType == 'notice'}">
						<ul>
							<li <c:if test="${searchVO.cate == ''}"> class=actived </c:if> ><a href="/eng/community/notice/community_list.do">ALL</a></li>
							<c:forEach var="codeResult" items="${ fnc:codeListSub('107') }" varStatus="status">
								<c:if test="${codeResult.code != '107202'}">
	                      			<li <c:if test="${searchVO.cate == codeResult.code}"> class='actived' </c:if> ><a href="/eng/community/notice/community_list.do?cate=${codeResult.code}"><c:out value="${codeResult.name}"/></a></li>
	                      		</c:if>
	                      	</c:forEach>
						</ul>
					</c:if>
					<c:if test="${searchVO.boardType == 'event'}">
						<ul>
							<li <c:if test="${searchVO.event_progress == ''}"> class=actived </c:if> ><a href="/eng/community/event/community_list.do">ALL</a></li>
							<li <c:if test="${searchVO.event_progress == 'P'}"> class='actived' </c:if> ><a href="/eng/community/event/community_list.do?event_progress=P">진행중 이벤트</a></li>
							<li <c:if test="${searchVO.event_progress == 'E'}"> class='actived' </c:if> ><a href="/eng/community/event/community_list.do?event_progress=E">종료된 이벤트</a></li>
						</ul>
					</c:if>
					<c:if test="${searchVO.boardType == 'sns'}">
						<ul>
					...
					<div class="media-list">
								...
								<c:choose>
									<c:when test="${not empty resultList }" >
										<c:forEach items="${resultList }" var="result" varStatus="status" >
											<li>
												<p class="img"><a href="javascript:void(0);" onclick="javascript:fnDetailView('/eng/community/${searchVO.boardType}/community_view.do', <c:out value="${result.seq }" />); return false;"><img src="${result.pc_thimg }" alt="${result.pc_alt}"/></a></p>
												...
												<p class="tit"><a href="javascript:void(0);" onclick="javascript:fnDetailView('/eng/community/${searchVO.boardType}/community_view.do', <c:out value="${result.seq }" />); return false;">
						...
						<c:if test="${ currentPage lt lastPage  }">
							<div class="btn-box mt10" id="noticeMoreDiv">
								<a href="javascript:void(0);" onclick="selectNoticeMore();" class="btn-02 type-03 btn-plus">MORE +</a>
							</div>
						</c:if>
						<!-- //btn-box -->
					</div>
					...
					<div class="event-list">
							...
							<ul id="eventUl">
								<c:choose>
									<c:when test="${not empty resultList }" >
										<c:forEach items="${resultList }" var="result" varStatus="status" >
											<li>
												<p class="img"><a href="javascript:void(0);" onclick="javascript:fnDetailView('/eng/community/${searchVO.boardType}/community_view.do', <c:out value="${result.seq }" />); return false;"><img alt="" src="${result.pc_thimg }"></a></p>
												<p class="ico-stamp">
												<span>
						...
						<c:if test="${ currentPage lt lastPage  }">
							<div class="btn-box mt10" id="eventMoreDiv">
								<a class="btn-02 type-03 btn-plus" href="javascript:void(0);" onclick="selectEventMore();">MORE  <span id="moreSpan">${ currentPage } / ${ lastPage }</span></a>
							</div>
						</c:if>
						<!-- //btn-box -->
					</div>
					...
					<div class="sns-list">
						...
						<c:if test="${ currentPage lt lastPage  }">
							<div class="btn-box mt80" id="snsMoreDiv">
								<a class="btn-02 type-03 btn-plus" href="javascript:void(0);" onclick="selectSnsMore();">MORE +</a>
							</div>
						</c:if>
						<!-- //btn-box -->
					</div>
...
<script src="/eng/common/js/front_ui.js"></script>
<script>
	$(function(){
	...
	function selectNoticeMore(){
		...
		$.ajax({
	     	url: "/eng/community/"+boardType+"/selectCommunityList.do",
	        type: "POST",
	        data: {
	        	'pageIndex' : pageIndex,
	        	'boardType' : boardType,
	        ...
	        success: function (data) {
	    	    ...
	    	    if(data.resultList.length > 0) {
					$.each(data.resultList, function(key, index){
						$html = '';
						$html += '<li>';
						$html += '<p class="img"><a href=\"javascript:void(0);\" onclick=\"javascript:fnDetailView(\'/eng/community/'+data.resultList[key].board_type+'/community_view.do\', \''+data.resultList[key].seq+'\' ); return false;"><img src="'+data.resultList[key].pc_thimg+'" alt=""/></a></p>';
						...
						$html += '</p>';
						$html += '<p class="tit">';
						$html += '<a href=\"javascript:void(0);\" onclick=\"javascript:fnDetailView(\'/eng/community/'+data.resultList[key].board_type+'/community_view.do\', \''+data.resultList[key].seq+'\' ); return false;">';
	        ...
	        success: function (data) {
	    	    ...
	    	    if(data.resultList.length > 0) {
					$.each(data.resultList, function(key, index){
						...
						$html = '';
						$html += '<li>';
						$html += '<p class="img"><a href="javascript:void(0);" onclick="javascript:fnDetailView(\'/eng/community/'+data.resultList[key].board_type+'/community_view.do\', \''+data.resultList[key].seq+'\' ); return false;"><img alt="" src="'+data.resultList[key].pc_thimg+'"></a></p>';
						...
						$html += '</span></p>';
						$html += '<p class="tit"><a href=\"javascript:void(0);\" onclick=\"javascript:fnDetailView(\'/eng/community/'+data.resultList[key].board_type+'/community_view.do\', \''+data.resultList[key].seq+'\' ); return false;">';
						$html += data.resultList[key].title+'';
						if(data.resultList[key].newYn == 'Y'){
							$html += ' <span class="new">new</span>';
						}
