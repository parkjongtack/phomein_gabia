...
<div id="wrap">
	...
	<div id="container">
		...
		<div id="content" class="store pd0"  style="width:100%;height:600px;">
			...
			<div class="store_search" style="z-index:20;">
				<h3>全店舗一覧</h3>
				<ul class="nav">
					<li><a href="#none" onclick="layerOpen('layerStoreName');">埋葬名</a></li><!-- 레이어 팝업 띄우기 : layerOpen('Id') -->
					<li><a href="#none" onclick="layerOpen('layerStoreAddr');">住所</a></li>
					<li><a href="#none" onclick="layerOpen('layerStoreOption');">オプション</a></li>
				</ul>
			</div>
		</div>
	</div>
	...
	<div id="layerStoreName" class="layer_pop" style="z-index:20001;">
		...
		<div class="cont">
			<div class="search_input">
				...
				<button type="button" class="btn_search" onclick="searchStoreNameBtn();"><span class="blind">検索</span></button>
			</div>
			<div class="search_result">
				<div class="result_option">
					<button type="button" class="btn_retry" onclick="searchStoreNameHiddenBtn();">再検索</button>
					<span class="num">検索結果 <em id="totalCnt">0</em> </span>
				</div>
				<!-- 검색 결과 없을 때 -->
				<div class="list_area no_result" id="storeListNameUl">
					<span>検索された情報がありません。</span>
				</div>
				<!-- //검색 결과 없을 때 -->
			</div>
		</div>
	...
	<div id="layerStoreAddr" class="layer_pop" style="z-index:20001;">
		...
		<div class="cont">
			<div class="search_input">
				<input type="text" class="text"  name="searchAddr" id="searchAddr" placeholder="住所" title="주소" onkeypress="if(event.keyCode==13) {searchAddrBtn();}">
				<input type="hidden" name="searchAddrHidden" id="searchAddrHidden">
				<button type="button" class="btn_search" onclick="searchAddrBtn();"><span class="blind">検索</span></button>
			</div>
			<div class="search_result">
				<div class="result_option">
					<button type="button" class="btn_retry" onclick="searchAddrHiddenBtn();">再検索</button>
					<span class="num">検索結果 <em id="totalAddrCnt">0</em> </span>
				</div>
				<!-- 검색 결과 없을 때 -->
		...
		<div class="cont">
			<div class="search_option">
				<ul>
					<li class="delivery">
						<span class="checkbox">
							<input type="checkbox" id="rd11" name="rd1" value="Y"><label for="rd11">出前</label>
						</span>
					</li>
					<li class="best">
						<span class="checkbox">
							<input type="checkbox" id="rd12" name="rd1" value="Y"><label for="rd12">最高売り場</label>
						</span>
					</li>
					<li class="parking">
						<span class="checkbox">
							<input type="checkbox" id="rd13" name="rd1" value="Y"><label for="rd13">駐車可能</label>
						</span>
					</li>
				</ul>
			</div>
			...
			<div class="search_result">
				<div class="result_option">
					<button type="button" class="btn_retry" onclick="searchOptBtn();">再検索</button>
					<span class="num">検索結果 <em id="totalOptCnt">0</em> </span>
				</div>
				<!-- 검색 결과 없을 때 -->
...
<!-- //wrap -->
<script src="/mjpn/common/js/swiper.min.js"></script>
<script src="/mjpn/common/js/common.js"></script>
</body>
</html>
<%--
	    ...
	    daum.maps.event.addListener(marker, 'click', function() {
			...
			$.ajax({
		        ...
		        success: function (data) {
		        	console.info(data);
		        	$('#detailName').text(data.result.en_name);
		        	var $html = "";
		        	...
		        	$('#detailEnaddr').text(data.result.enaddr);
...
<script>
	...
	$(document).ready(function () {
		...
		if($('#searchStoreName').val() != ""){
			...
			store_init('The Head Store in Jeongja');
			$("#init").val('Y');
		}
	});
	...
	var store_init = function (storeName){
		$.ajax({
	     	url: "/mjpn/store/selectStoreList.do",
	        type: "POST",
	        data: {
	        	'en_name' : storeName,
	        	'useyn' : 1
	        ...
	        success: function (data) {
	    	    ...
	    	    if(data.resultList.length > 0) {
    				...
    				$.each(data.resultList, function(key, index){
    					...
    					$html += '<span class="tit">'+data.resultList[key].en_name+'</span>';
    					$html += '<span class="tel">'+data.resultList[key].hp1+'-'+data.resultList[key].hp2+'-'+data.resultList[key].hp3+'</span>';
    					$html += '<span class="addr">'+data.resultList[key].enaddr+'</span>';
		...
		$.ajax({
	        ...
	        success: function (data) {
	    	    ...
	    	    if(data.resultList.length > 0) {
	    	    	...
	    	    	$html += '<ul>';
    				$.each(data.resultList, function(key, index){
    					console.info(data.resultList[key]);
    					$html += '<li>';
    					$html += '<a href="javascript:void(0);" class="link_detail" onclick="initDaumApi(\''+data.resultList[key].seq +'\' , \''+data.resultList[key].lat +'\' , \''+ data.resultList[key].longi+'\')">';
    					$html += '<span class="tit">'+data.resultList[key].en_name+'</span>';
    					$html += '<span class="tel">'+data.resultList[key].hp1+'-'+data.resultList[key].hp2+'-'+data.resultList[key].hp3+'</span>';
