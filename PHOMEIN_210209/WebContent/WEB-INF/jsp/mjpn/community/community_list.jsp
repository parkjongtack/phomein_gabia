...
<div id="wrap">
	...
	<div id="container">
		...
		<!-- //lnb -->
		<!-- content -->
		<form id="searchForm" name="searchForm" method="post" action="/mjpn/community/${searchVO.boardType}/community_list.do">
						...
						<ul class="nav_cate_wrap">
							...
							<c:forEach var="codeResult" items="${ fnc:codeListSub('107') }" varStatus="status">
								<c:if test="${codeResult.code != '107202'}">
									<li class="nav_cate_btn <c:if test="${searchVO.cate == codeResult.code}">on</c:if>"><a href="/mjpn/community/notice/community_list.do?cate=${codeResult.code}"><c:out value="${codeResult.name}"/></a></li>
								</c:if>
							</c:forEach>
						</ul>
	 				</div>
								...
								<c:forEach items="${resultList }" var="result" varStatus="status" >
									<li>
										<a href="javascript:void(0);"
											onclick="javascript:fnDetailView('/mjpn/community/${searchVO.boardType}/community_view.do', <c:out value="${result.seq }" />); return false;"
		...
		<c:if test="${searchVO.boardType == 'event'}">
			<div id="content" class="community">
				...
				<div class="list_area">
					...
					<div class="nav_cate swiper">
						<ul class="nav_cate_wrap">
							<li class="nav_cate_btn <c:if test="${searchVO.event_progress == ''}">on</c:if>"><a href="/mjpn/community/event/community_list.do">전체</a></li>
							<li class="nav_cate_btn <c:if test="${searchVO.event_progress == 'P'}">on</c:if>"><a href="/mjpn/community/event/community_list.do?event_progress=P">진행중 이벤트</a></li>
							<li class="nav_cate_btn <c:if test="${searchVO.event_progress == 'E'}">on</c:if>"><a href="/mjpn/community/event/community_list.do?event_progress=E">종료된 이벤트</a></li>
						</ul>
	 				</div>
					<!-- //카테고리 메뉴 -->
					<c:choose>
			...
			<div id="content" class="community">
					...
					<ul class="nav_cate_wrap">
						...
						<c:forEach var="codeResult" items="${ fnc:codeListSub('109') }" varStatus="status">
							<c:if test="${codeResult.code == '109203'}">
								<li class="nav_cate_btn <c:if test="${searchVO.cate == codeResult.code}">on</c:if>"><a href="/mjpn/community/sns/community_list.do?cate=${codeResult.code}"><c:out value="${codeResult.name}"/></a></li>
							</c:if>
						</c:forEach>
					</ul>
 				</div> -->
				...
				<c:if test="${ currentPage lt lastPage  }">
					<div class="btn_area" id="snsMoreDiv">
						<button type="button" class="btn_tyf c_white btn_more" onclick="selectSnsMore();">もっと見る</button>
					</div>
				</c:if>
				<!-- //버튼 영역 -->
...
<!-- //wrap -->
<script src="/mjpn/common/js/jquery-1.12.4.min.js"></script>
...
// 카테고리 메뉴
	...
	function selectNoticeMore(){
		...
		$.ajax({
	     	url: "/mjpn/community/"+boardType+"/selectCommunityList.do",
	        type: "POST",
	        data: {
	        	'pageIndex' : pageIndex,
	        	'boardType' : boardType,
	        ...
	        success: function (data) {
	    	    ...
	    	    if(data.resultList.length > 0) {
					$.each(data.resultList, function(key, index){
						...
						$html += 'onclick="javascript:fnDetailView(\'/mjpn/community/'+data.resultList[key].board_type+'/community_view.do\', \''+data.resultList[key].seq+'\'); return false;"';
