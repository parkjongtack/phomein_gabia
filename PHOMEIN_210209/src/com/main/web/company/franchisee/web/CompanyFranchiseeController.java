package com.main.web.company.franchisee.web;
...
@Controller
@RequestMapping({ "/company/franchisee/*", "/mcompany/franchisee/*" })
public class CompanyFranchiseeController {

	private final Logger logger = LoggerFactory.getLogger(getClass());

	private final String NAMESPACE = "company/franchisee/";

	private final String M_NAMESPACE = "mcompany/franchisee/";
	...
	@RequestMapping(value = { "estimate.do", "cost.do" }, method = { RequestMethod.GET, RequestMethod.POST })
	public String estimate(@RequestParam Map param, HttpSession session, ModelMap model, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		String deviceType = getDeviceType(request);

		if(!deviceType.equals("normal")){
			return M_NAMESPACE + "cost";
		}
	...
	@RequestMapping(value = "interview.do", method = { RequestMethod.GET, RequestMethod.POST })
	public String interview(@RequestParam Map param,
			@RequestParam(required = false, defaultValue = "1") String pageIndex, HttpSession session, ModelMap model,
			HttpServletRequest request, HttpServletResponse response) throws Exception {
		String deviceType = getDeviceType(request);
	...
	public @ResponseBody Map<String, Object> get_interview_list_ajax(@RequestParam Map param,
		...
		param.put("pageUnit", 9999);
		param.put("startCount", 0);
		returnMap.put("searchTotalCnt", interviewService.selectPaging(param).size());
		returnMap.put("pageIndex", pageIndex);
		return returnMap;
	}
	...
	public String info(@RequestParam Map param, HttpSession session, ModelMap model, HttpServletRequest request,
		...
		if(!deviceType.equals("normal")){
			if (StringUtils.isEmpty(chk)) {
				return "redirect:/mcompany/franchisee/terms.do";
			}
		}

		if (StringUtils.isEmpty(chk)) {
			return "redirect:/company/franchisee/terms.do";
		}
